...
int WINAPI MessageBoxW_hook(
	...
	UINT	uType)
{
	std::wstring_view text(lpText);
	if (text.contains(L"fake Serial Number"sv) || text.starts_with(L"The main IDM executive file is damaged."sv))
	{
		return 0;
	}
	return MessageBoxW(hWnd, std::format(L"MessageBoxW (hooked):\n{}", text).c_str(), lpCaption, uType);
}
