...
struct formatter<vector<BYTE>, CharT> : range_formatter<BYTE, CharT> {
...
inline void acquire_privileges()
{
	...
	{
		std::println(TEXT("{:{}}: {:#010x}"), name, max_width, attr);
	}
	std::println();
...
void priv_getkv(std::tstring_view path)
{
	...
	for (DWORD i{};; ++i)
	{
		...
		std::println("[{}]", i + 1);
		std::println(TEXT("Name: {:.{}}"), name, name_len);
		std::println(TEXT("Type: {} (REG_{})"), type, reg_type_map.contains(type) ? reg_type_map.at(type) : TEXT("<unknown>"sv));
		if (data_len == 0)
		{
		...
		switch (type)
		{
			case REG_SZ:
			case REG_EXPAND_SZ:
				std::println(TEXT("Value: {}"), reinterpret_cast<LPTSTR>(data.data()));
				break;
			case REG_DWORD:
			...
			case REG_DWORD_BIG_ENDIAN:
				...
				break;
			case REG_LINK:
				std::println(L"Value: {:.{}}", reinterpret_cast<LPCWSTR>(data.data()), data_len / sizeof(WCHAR));
				break;
			case REG_MULTI_SZ:
				std::println(TEXT("Value: {::?s}"),
...
inline void print_ace(AceType *ace)
{
	...
	std::println(TEXT("Access: {:#010x} {}"), ace->Mask,
				 ace_access_map<SeObjectType> | std::views::filter([ace](const auto &pair) { return (ace->Mask & pair.first) != 0; }) | std::views::values);
	std::println(TEXT("User: {} (SID: {}) (Type: {})"), sid_info.name, LPTSTR(sid_info.str), sid_info.type);
}
...
inline void print_acl(PACL acl)
{
	...
	for (DWORD i{}; i < acl_size_info.AceCount; ++i)
	{
		...
		std::println("[{}]", i + 1);
		std::println(TEXT("Flags: {:#04x} {}"), ace_header->AceFlags,
...
try
{
	...
	{
		std::println("Usage:");
		std::println(TEXT("(1)  \"{}\" read <infile> <outfile>"), argv[0]);
		std::println(TEXT("(2)  \"{}\" write <infile> <outfile>"), argv[0]);
		std::println(TEXT("(3)  \"{}\" list <directory>"), argv[0]);
		std::println(TEXT("(4)  \"{}\" mkdir <directory>"), argv[0]);
		std::println(TEXT("(5)  \"{}\" remove <file>"), argv[0]);
		std::println(TEXT("(6)  \"{}\" move <old path> <new path>"), argv[0]);
		std::println(TEXT("(7)  \"{}\" getkv <registry>"), argv[0]);
		std::println(TEXT("(8)  \"{}\" setkv <registry> <name> <type> [<byte> ...]"), argv[0]);
		std::println(TEXT("     <type>: {}"), reg_type_map | std::views::values);
		std::println(TEXT("     <byte>: 00-ff"));
		std::println(TEXT("(9)  \"{}\" delkv <registry> <name>"), argv[0]);
		std::println(TEXT("(10) \"{}\" listkey <registry>"), argv[0]);
		std::println(TEXT("(11) \"{}\" delkey <registry>"), argv[0]);
		std::println(TEXT("(12) \"{}\" renkey <registry> <new name>"), argv[0]);
		std::println(TEXT("(13) \"{}\" getfacl|getkacl <file|registry>"), argv[0]);
		std::println(TEXT("(14) \"{}\" setf(i)acl|setk(i)acl <file|registry> <sddl>"), argv[0]);
		return 0;
	}
